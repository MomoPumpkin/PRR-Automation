...
/**
 * Template for rendering Production Reliability Review (PRR) results using a GitHub-like markdown approach
 * 
 * @param {Object} props - Component props
 * @param {Object} props.data - The structured data from the analysis
 * @returns {JSX.Element} The rendered template
 */
const PRRTemplate = ({ data }) => {
  if (!data) {
  ...
  const formatRawText = (text) => {
    ...
    if (!formattedText.startsWith('# ')) {
      formattedText = '# Production Reliability Review (PRR)\n\n' + formattedText;
    }
  ...
  return (
    <div className="prr-template">
      <MarkdownRenderer content={formatRawText(data.raw_text)} />
    </div>
  );
};

export default PRRTemplate;